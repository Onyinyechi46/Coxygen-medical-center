...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM user_wallets WHERE user_id = ? AND status = 'verified'");
$stmt->execute([$userId]);
$hasVerifiedWallet = ((int)$stmt->fetchColumn()) > 0;

$stmt = $pdo->prepare("SELECT 1 FROM user_wallets WHERE user_id = ? AND wallet_address_hash = ? AND status = 'verified' LIMIT 1");
